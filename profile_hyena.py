...
def run_experiment(config, seed, device):
    ...
    run_config = OmegaConf.load(os.path.join(config.trainer.checkpoints_path, "config.yaml"))
    discretizer = torch.load(os.path.join(config.trainer.checkpoints_path, "discretizer.pt"), map_location=device)

    model = TrajectoryModel(layer_type="hyena", **config.model)
    model.eval()
    model.to(device)
    model.load_state_dict(torch.load(os.path.join(config.trainer.checkpoints_path, "model_8.pt"), map_location=device))
